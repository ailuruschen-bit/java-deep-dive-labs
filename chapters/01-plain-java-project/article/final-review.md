...
- 日文、英文：分别由独立写作者按母语技术文章习惯撰写，再由独立审稿人完整审读两稿；主编辑核对实验一致性并落实修改。
...
- 英文将“没有编译 classpath”改为“没有显式配置编译 classpath”，避免否定默认搜索位置。
- 三篇明确应用类与资源进入 `target/classes`、测试类与资源进入 `target/test-classes`，消除“分别”紧随源码与资源造成的错误对应。
- 日文将中文直译的“主コード／主ソース”改为自然的应用代码表达；修正遗漏搜索位置与异常报告的用语。
- 两篇补齐 lab 入口、JLS 和 IDE 官方参考；Boot 段统一说明创建支持嵌套 JAR 的类加载器。
- 两篇图中的 JAR 条目明确为 classpath 条目，避免与归档内部条目混淆。
- 自动比对确认：三篇均有 26 个相同的可执行代码／命令块（忽略本地化注释），均引用相同的 11 张真实截图，图片文件完整；各有 54 个闭合代码块。
- 两位语言写作者在文件保存后遇到用量限制；没有把失败通知当作完稿审核结果。后续全文审核由独立审稿人与主编辑完成。
